...
parse_usage() {
    ...
    if echo "$json_data" | jq -e '.limited_user_quotas' >/dev/null 2>&1; then
        ...
        if [[ "$USE_COLOR" == "true" ]]; then
            ...
            echo "  ${cyan}2. gpt-5.3-codex 1x${reset}"
            echo "  ${cyan}3. gpt-5.2-codex 1x${reset}"
            echo "  ${cyan}4. gpt-5.2 1x${reset}"
            echo "  ${cyan}5. gpt-5.1 1x${reset}"
            echo "  ${cyan}6. gpt-5.4-mini 0.33x${reset}"
            echo "  ${cyan}7. gpt-5-mini 0x${reset}"
            echo "  ${cyan}8. gpt-4.1 0x${reset}"
            echo "  ${cyan}9. claude-sonnet-4.6 1x${reset}"
            echo "  ${cyan}10. claude-sonnet-4.5 1x${reset}"
            echo "  ${cyan}11. claude-haiku-4.5 ✓ 0.33x${reset}"
            echo "  ${cyan}12. claude-opus-4.6 3x${reset}"
            echo "  ${cyan}13. claude-opus-4.5 3x${reset}"
            echo "  ${cyan}14. claude-sonnet-4 1x${reset}"
            echo ""
            echo "${yellow}💡 Note: Models need to be enabled at GitHub Copilot Features Settings${reset}"
            echo "${yellow}   (https://github.com/settings/copilot/features) before they become available.${reset}"
            echo "${yellow}   If you choose a disabled model, 'copilot' will prompt you to enable it.${reset}"
            ...
            echo "  2. gpt-5.3-codex 1x"
            echo "  3. gpt-5.2-codex 1x"
            echo "  4. gpt-5.2 1x"
            echo "  5. gpt-5.1 1x"
            echo "  6. gpt-5.4-mini 0.33x"
            echo "  7. gpt-5-mini 0x"
            echo "  8. gpt-4.1 0x"
            echo "  9. claude-sonnet-4.6 1x"
            echo "  10. claude-sonnet-4.5 1x"
            echo "  11. claude-haiku-4.5 ✓ 0.33x"
            echo "  12. claude-opus-4.6 3x"
            echo "  13. claude-opus-4.5 3x"
            echo "  14. claude-sonnet-4 1x"
            echo ""
            echo "💡 Note: Models need to be enabled at GitHub Copilot Features Settings"
            echo "   (https://github.com/settings/copilot/features) before they become available."
            echo "   If you choose a disabled model, 'copilot' will prompt you to enable it."
