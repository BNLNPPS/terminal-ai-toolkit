...
parse_usage() {
    ...
    if echo "$json_data" | jq -e '.limited_user_quotas' >/dev/null 2>&1; then
        ...
        if [[ "$USE_COLOR" == "true" ]]; then
            ...
            echo "  ${cyan}5. claude-sonnet-4 1x${reset}"
            echo "  ${cyan}6. gpt-5.2-codex 1x${reset}"
            echo "  ${cyan}7. gpt-5.1-codex-max 1x${reset}"
            echo "  ${cyan}8. gpt-5.1-codex 1x${reset}"
            echo "  ${cyan}9. gpt-5.2 1x${reset}"
            echo "  ${cyan}10. gpt-5.1 1x${reset}"
            echo "  ${cyan}11. gpt-5 1x${reset}"
            echo "  ${cyan}12. gpt-5.1-codex-mini 0.33x${reset}"
            echo "  ${cyan}13. gpt-5-mini 0x${reset}"
            echo "  ${cyan}14. gpt-4.1 0x${reset}"
            echo "  ${cyan}15. gemini-3-pro-preview 1x${reset}"
            echo ""
            echo "${yellow}💡 Note: Models need to be enabled at GitHub Copilot Features Settings${reset}"
            echo "${yellow}   (https://github.com/settings/copilot/features) before they become available.${reset}"
            echo "${yellow}   If you choose a disabled model, 'copilot' will prompt you to enable it.${reset}"
            ...
            echo "  5. claude-sonnet-4 1x"
            echo "  6. gpt-5.2-codex 1x"
            echo "  7. gpt-5.1-codex-max 1x"
            echo "  8. gpt-5.1-codex 1x"
            echo "  9. gpt-5.2 1x"
            echo "  10. gpt-5.1 1x"
            echo "  11. gpt-5 1x"
            echo "  12. gpt-5.1-codex-mini 0.33x"
            echo "  13. gpt-5-mini 0x"
            echo "  14. gpt-4.1 0x"
            echo "  15. gemini-3-pro-preview 1x"
            echo ""
            echo "💡 Note: Models need to be enabled at GitHub Copilot Features Settings"
            echo "   (https://github.com/settings/copilot/features) before they become available."
            echo "   If you choose a disabled model, 'copilot' will prompt you to enable it."
